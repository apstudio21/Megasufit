...
(function () {
  ...
  function cardThumbHTML(item, index) {
    ...
    const isVid = isVideo(first.file);
    const thumb = isVid
      ? `<video src="${first.file}"${first.poster ? ` poster="${first.poster}"` : ''} muted preload="metadata"></video>`
      : '';
    const bgStyle = !isVid && first.file ? ` style="background-image:url('${first.file}')"` : '';

    return `<button type="button" class="gallery-card" data-card="${index}"><div class="gallery-card__inner"${bgStyle}>${thumb}</div>${badge}</button>`;
  }
